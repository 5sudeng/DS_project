...
def _load_kv_file(filename: str) -> dict:
    """Load simple KEY=VALUE lines from a file under project root."""
...
def load_openai_voice_env() -> dict:
    """Load OpenAI voice config from openai_voice_env.txt or environment."""
    data = _load_kv_file("openai_voice_env.txt")
    return {
        "api_key": data.get("OPENAI_API_KEY") or os.getenv("OPENAI_VOICE_API_KEY") or os.getenv("OPENAI_API_KEY"),
        "base_url": data.get("OPENAI_BASE_URL") or os.getenv("OPENAI_BASE_URL"),
        "stt_model": data.get("OPENAI_STT_MODEL") or data.get("STT_MODEL") or os.getenv("OPENAI_STT_MODEL"),
        "path": data.get("__path__"),
    }


def load_rtzr_voice_env() -> dict:
    """Load RTZR credentials from rtzr_voice_env.txt or environment."""
    data = _load_kv_file("rtzr_voice_env.txt")
    return {
        "client_id": data.get("RTZR_CLIENT_ID") or os.getenv("RTZR_CLIENT_ID"),
        "client_secret": data.get("RTZR_CLIENT_SECRET") or os.getenv("RTZR_CLIENT_SECRET"),
        "path": data.get("__path__"),
    }
...
class SpeechInterface:
    ...
    def __init__(
        ...
        *,
        ...
        env_cfg = load_openai_voice_env()
        resolved_key = openai_api_key or env_cfg.get("api_key") or os.getenv("OPENAI_API_KEY")
        resolved_base = base_url or env_cfg.get("base_url") or os.getenv("OPENAI_BASE_URL") or "https://api.openai.com/v1"
        resolved_model = stt_model or env_cfg.get("stt_model") or "whisper-1"

        if not resolved_key:
            raise RuntimeError(
                "OpenAI 음성 API 키를 찾을 수 없습니다. openai_voice_env.txt 또는 OPENAI_API_KEY를 확인하세요."
            )
...
class RTZRSpeechInterface:
    ...
    def __init__(
        ...
        *,
        ...
        env_cfg = load_rtzr_voice_env()
        self.client_id = client_id or env_cfg.get("client_id") or os.getenv("RTZR_CLIENT_ID")
        self.client_secret = client_secret or env_cfg.get("client_secret") or os.getenv("RTZR_CLIENT_SECRET")
        if not self.client_id or not self.client_secret:
            raise RuntimeError("RTZR_CLIENT_ID/RTZR_CLIENT_SECRET 가 필요합니다. rtzr_voice_env.txt를 확인하세요.")
...
def build_io_interface(
    *,
    ...
    if backend == "rtzr":
        creds = load_rtzr_voice_env()
        # 새 RTZR gRPC 기반 인터페이스
        return RTZRSpeechInterface(
            client_id=rtzr_client_id or creds.get("client_id"),
            client_secret=rtzr_client_secret or creds.get("client_secret"),
        )

    env_cfg = load_openai_voice_env()
    return SpeechInterface(
        openai_api_key=openai_api_key or env_cfg.get("api_key"),
        stt_model=stt_model or env_cfg.get("stt_model") or "whisper-1",
        use_keyboard_input=use_keyboard_input,
        base_url=base_url or env_cfg.get("base_url"),
    )
